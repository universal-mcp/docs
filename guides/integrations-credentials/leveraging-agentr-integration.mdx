...
2.  **Getting Credentials:** When an `Application` needs credentials (e.g., `my_github_app.integration.get_credentials()`), the `AgentRIntegration` uses its [`AgentrClient`](https://github.com/universal-mcp/universal-mcp/blob/master/src/universal_mcp/utils/agentr.py) to make an API call to `https://api.agentr.dev/api/{integration_name}/credentials/`.
...
**Example from [`examples/reddit_summary.py`](https://github.com/universal-mcp/examples/blob/master/agents/reddit_summary.py):**
```python
