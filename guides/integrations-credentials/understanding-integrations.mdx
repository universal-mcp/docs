...
All specific integration types in Universal MCP inherit from the `Integration` abstract base class (defined in `integrations/integration.py`). This class defines a common interface with key methods: [1]
...
*   **`ApiKeyIntegration`**: For services that use simple API key-based authentication. [1]
*   **`OAuthIntegration`**: For services that use OAuth 2.0 flows (though the current implementation in `integration.py` seems to be a more general OAuth outline and might need further details for a full 2.0 flow with refresh tokens, etc.).
*   **`AgentRIntegration`**: A specialized integration for services managed through the AgentR platform. It uses an AgentR API key to securely fetch service-specific credentials managed by AgentR. [1, 2]
...
In the `ServerConfig` (e.g., in your `local_config.json`), you can also define integrations for specific applications. [3]

The `integrations/README.md` file provides a high-level overview of these concepts. [4]

```
[1] integrations/integration.py - Integration class, ApiKeyIntegration, AgentRIntegration
[2] utils/agentr.py - AgentrClient class
[3] config.py - IntegrationConfig within AppConfig
