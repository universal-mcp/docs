...
4.  **Convert Tools to Langchain Format:**
    ...
    ```
    This uses the `convert_tool_to_langchain_tool` adapter internally, which wraps your MCP tool's `run` method within a Langchain `StructuredTool`. [1]
...
The `examples/langraph.py` script demonstrates a very simple agent with a custom `calculate` tool: [2]
...
```
 tools/adapters.py - convert_tool_to_langchain_tool, ToolFormat
 exmples.xml - examples/langraph.py
```