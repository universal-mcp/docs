...
2.  **Write a Clear Docstring:** The docstring is crucial. Universal MCP parses it to generate the tool's `description`, `args_description`, `returns_description`, and `tags`. Follow a style that [`docstring_parser`](https://github.com/universal-mcp/universal-mcp/blob/master/src/universal_mcp/utils/docstring_parser.py) can understand (Google style is recommended).
...
1.  **`ToolManager.add_tool(fn: Callable[..., Any] | Tool, name: str | None = None) -> Tool`**:
    ...
    *   This method internally calls [`Tool.from_function(fn, name=name)`](https://github.com/universal-mcp/universal-mcp/blob/master/src/universal_mcp/tools/tools.py) to create the `Tool` object.
...
2.  **Creating a `Tool` instance explicitly and then registering:**
    ...
    ```
    The [`examples/langraph.py`](https://github.com/universal-mcp/examples/blob/master/agents/langraph.py) file demonstrates adding a simple `calculate` function as a tool.
