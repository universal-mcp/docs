...
If you omit the `--config` option, the server will attempt to start with default `ServerConfig` values, which might not load any specific applications unless defined via environment variables or other default mechanisms. Refer to [`cli.py`](https://github.com/universal-mcp/universal-mcp/blob/master/src/universal_mcp/cli.py) for more information.
