...
For these guides, we will primarily focus on the `LocalServer` as it's excellent for development and understanding the core mechanics.

With your configuration file ready, you're set to launch the server!

```
[1] config.py
[2] servers/server.py - ServerConfig type field
[3] servers/server.py - LocalServer._load_apps method
[4] servers/server.py - AgentRServer._fetch_apps method
```