...
Replace `<app_name>` with the name of the application you wish to install (e.g., `claude`, `cursor`, `github`). This command typically configures your local environment to point to the Universal MCP system, often involving an AgentR API key.
Refer to [`cli.py`](https://github.com/universal-mcp/universal-mcp/blob/master/src/universal_mcp/cli.py) for more information.

For a list of currently supported applications for this installation method, you can often find them by running the command without an app name or with a non-existent one, or refer to the [`utils/installation.py`](https://github.com/universal-mcp/universal-mcp/blob/master/src/universal_mcp/utils/installation.py) file in the SDK.
