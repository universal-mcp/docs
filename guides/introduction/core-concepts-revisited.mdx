...
Universal MCP provides base classes like `BaseApplication`, `APIApplication` (for HTTP APIs), and `GraphQLApplication` (for GraphQL APIs) to simplify the creation of new application integrations. [1]
...
Key features of Actions include dynamic function calling, structured inputs and outputs, integration with various authentication methods, error handling, and tool management. You create custom actions by defining methods within an application class. [1]
...
The system supports various integration types, including API Key, OAuth, and Basic Authentication. [1]
...
When these specific events occur, Triggers notify the AI agents, allowing them to respond proactively and intelligently. [1]
